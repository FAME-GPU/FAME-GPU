    ...
    string flag_CompType, PROFILE* Profile)
{
    ...
    switch(isInit)
    {
        case 0:
        {
            ...
            PC_cublas_dot(cuHandles.cublas_handle, Asize, U+Asize, 1, U, 1, &alpha_tmp);

            T0[0] = alpha_tmp.x;

            cublas_zcale = make_cucmpx(-T0[0], 0.0);
            PC_cublas_axpy(cuHandles.cublas_handle, Asize, &cublas_zcale, U, 1, U+Asize, 1);

            PC_cublas_nrm2(cuHandles.cublas_handle, Asize, U+Asize, 1, &T1[0]);

            cublas_scale = 1.0 / T1[0];
            PC_cublas_dscal(cuHandles.cublas_handle, Asize, &cublas_scale, U+Asize, 1);
    ...
    for(j = loopStart; j < Nstep; j++)
    {
        ...
        PC_cublas_dot(cuHandles.cublas_handle, Asize, U+Asize*(j+1), 1, U+Asize*j, 1, &alpha_tmp);

        T0[j] = alpha_tmp.x;

        cublas_zcale =  make_cucmpx(-alpha_tmp.x, 0.0);
        PC_cublas_axpy(cuHandles.cublas_handle, Asize, &cublas_zcale, U+Asize*j, 1, U+Asize*(j+1), 1);

        cublas_zcale =  make_cucmpx(-T1[j - 1], 0.0);
        PC_cublas_axpy(cuHandles.cublas_handle, Asize, &cublas_zcale, U+Asize*(j-1), 1, U+Asize*(j+1), 1);

        PC_cublas_nrm2(cuHandles.cublas_handle, Asize, U+Asize*(j+1), 1, &T1[j]);
        
        cublas_scale = 1.0 / T1[j];
        PC_cublas_dscal(cuHandles.cublas_handle, Asize, &cublas_scale, U+Asize*(j+1), 1);

        /* Full Reorthogonalization */
        for( i = 0; i <= j; i++)
        {
            PC_cublas_dot(cuHandles.cublas_handle, Asize, U+Asize*i, 1, U+Asize*(j+1), 1, &Loss);
            
            cublas_zcale =  make_cucmpx(-Loss.x, -Loss.y);
            PC_cublas_axpy(cuHandles.cublas_handle, Asize, &cublas_zcale, U+Asize*i, 1, U+Asize*(j+1), 1);
        }
    }
